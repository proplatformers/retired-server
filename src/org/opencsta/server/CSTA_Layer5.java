...
public class CSTA_Layer5 implements CSTA_Layer_Interface, Runnable {
    ...
    //@todo fix up this link thread bizo - chuck it into csta link implm. class.
    private boolean CSTALinkUp ;
    //This main method is used for doing only layer 5 testing, usually
    //during initial development on a new switch, to work out how to log onto it.
    public static void main(String[] args){
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(CSTAServer.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
    ...
    //This constructor is used with the main method, and is to be used
    //only for initial development/integration with a new switch.
    public CSTA_Layer5(Properties _props){
        ...
        workListUpward = Collections.synchronizedList( new LinkedList<StringBuffer>() );
        workListDownward = Collections.synchronizedList( new LinkedList<StringBuffer>() );
...
//        link = new CSTA_Link_Siemens_Hipath3000_Network(this) ;
    ...
    public CSTA_Layer5(CSTA_Layer7 newLayer7,Properties _props) throws NoCSTAImplementationException{
        ...
        workListDownward = Collections.synchronizedList( new LinkedList<StringBuffer>() );
        alog.info("Worklists --- starting CSTA_Link implementation") ;
        if( theProps.getProperty("CSTASERVER_PBX_VENDOR").equalsIgnoreCase("SIEMENS") ){
            alog.info("PBX Vendor: Siemens");
            if( theProps.getProperty("CSTASERVER_PBX_MODEL").equalsIgnoreCase("HIPATH3000") ){
                alog.info("PBX Model: Hipath 3000") ;
                if( theProps.getProperty("CSTASERVER_PBX_IMPLEMENTATION").equalsIgnoreCase("SERIAL") ){
                    alog.info("PBX CSTA Implementation: Serial" ) ;
                    link = new CSTA_Link_Siemens_Hipath3000_Serial(this) ;
                }
                else if( theProps.getProperty("CSTASERVER_PBX_IMPLEMENTATION").equalsIgnoreCase("NETWORK") ){
    ...
    //Essential for the program - come back and do.
    ...
    //Layer 5 has it's own thread for processing jobs/messages in both direction
...
//        addWorkOUT(new StringBuffer(str_ACSE_req)) ;
    ...
    public  boolean PassedUp(StringBuffer curInStr){
        ...
        alog.info(this.getClass().getName() + " ---> " + " Entered Layer 5 R: " + tmplogStr) ;
    	...
    	if( isLoggedIn() ){
    		...
    		if( isLoginACSERequestSent() ){
    			...
    			if( isLoginACSEResponseReceived() ){
    				//we've received the response, have we received the follow up string
    				alog.info("\nQ3) We know we've received the ACSE Response, now what?") ;
    				if( isLoginFirstROSEInvokeReceived() ){
    					//we've received the first ROSE invoke, have we sent a reply
    					...
    					//is this the First ROSE Invoke, check it, set FirstROSEInvokeReceived to true
    				...
    				//is this the ACSE Response Received?  check it, set ACSERseponseReceived to true
    ...
    private StringBuffer Wrap(StringBuffer curOutStr){
        curOutStr = curOutStr.insert(0,(char)curOutStr.length()).insert(0,'\u0080').insert(0,'\u0026') ;
        return curOutStr ;
    }
	...
	public synchronized void addWorkOUT(StringBuffer job){
        ...
        alog.warn(this.getClass().getName() + " added work out -- need to notify link thread") ;
	}
    ...
    private boolean startCSTALink(){
        alog.info(this.getClass().getName() + " .... CSTA Link first attempt to connect" ) ;
        int tmp_counter = 0 ;
